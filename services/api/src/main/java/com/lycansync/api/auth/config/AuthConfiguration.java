...
public class AuthConfiguration {
    ...
    SecurityFilterChain authSecurity(HttpSecurity http, AuthService service, ObjectMapper json) throws Exception {
        ...
        http.csrf(csrf -> csrf.disable())
                ...
                .exceptionHandling(errors -> errors
                        .authenticationEntryPoint((request, response, exception) -> writeProblem(json, response, 401, "请先登录"))
                        .accessDeniedHandler((request, response, exception) -> writeProblem(json, response, 403, "没有访问权限")))
                .addFilterBefore(new OncePerRequestFilter() {
                    ...
                    protected void doFilterInternal(HttpServletRequest request, HttpServletResponse response,
                        ...
                        if (loginAttempt && !allowAttempt()) {
                            writeProblem(json, response, 429, "登录请求过于频繁，请一分钟后重试");
                            return;
                        }
                        ...
                        if (header != null) {
                            try {
                                if (!header.startsWith("Bearer ")) throw new AuthException(
                                        HttpStatus.UNAUTHORIZED, "认证格式无效");
                                AuthUser user = service.authenticate(header.substring(7));
                                SecurityContextHolder.getContext().setAuthentication(
                                        new PreAuthenticatedAuthenticationToken(user, null, List.of()));
                            } catch (AuthException exception) {
                                writeProblem(json, response, exception.getStatus().value(), exception.getMessage());
                                return;
                            } catch (DataAccessException exception) {
                                writeProblem(json, response, 503, "认证服务暂时不可用");
                                return;
                            }
                        }
    ...
    private static void writeProblem(ObjectMapper json, HttpServletResponse response, int status, String message)
            throws IOException {
        ...
        problem.setProperty("code", "AUTH_ERROR");
