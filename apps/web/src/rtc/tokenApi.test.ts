...
describe('申请 RTC 入房凭证', () => {
  it('通过同源 POST 请求凭证，不缓存或携带 cookie', async () => {
    ...
    await expect(requestRtcToken('pack', signal)).resolves.toEqual(credential);
    expect(fetchMock).toHaveBeenCalledWith('/api/rtc/token', expect.objectContaining({
      method: 'POST', body: '{"groupId":"pack"}', cache: 'no-store', credentials: 'omit', signal,
    }));
  });
  ...
  it.each([
    ...
    await expect(requestRtcToken('pack', new AbortController().signal)).rejects.toThrow(String(message));
  });

  it('拒绝开发代理返回的 HTML', async () => {
    vi.stubGlobal('fetch', vi.fn().mockResolvedValue(new Response('<html>proxy</html>')));
    await expect(requestRtcToken('pack', new AbortController().signal)).rejects.toThrow('有效 JSON');
  });
  ...
  it.each([
    ...
    await expect(requestRtcToken('pack', new AbortController().signal)).rejects.toThrow('缺少必要字段');
  });

  it('保留取消请求的异常，交给会话逻辑判断是否仍需提示', async () => {
    const aborted = new DOMException('aborted', 'AbortError');
    vi.stubGlobal('fetch', vi.fn().mockRejectedValue(aborted));
    await expect(requestRtcToken('pack', new AbortController().signal)).rejects.toBe(aborted);
  });
});
