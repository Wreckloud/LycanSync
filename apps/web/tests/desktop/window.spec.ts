...
test('桌面窗口安全边界、真实控件与退出', async () => {
  ...
  try {
    ...
    await expect(page.locator('.members-panel')).toContainText('成员列表尚未开放');
    ...
    await page.locator('.message-list').evaluate((list) => {
      const message = list.firstElementChild!;
      for (let index = 0; index < 40; index++) list.append(message.cloneNode(true));
    });
